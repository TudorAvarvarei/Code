...
class ModelController(BaseModel):
    def __init__(self, e1, e2, e3, e4, p1, mask=None):
        super().__init__()

        self.e1 = MaskedLinearCubaLif(**e1, mask=torch.t(mask[0]))
        self.e2 = LinearCubaLif(**e2)
        self.e3 = LinearCubaLif(**e3)
        self.e4 = LinearCubaLif(**e4)
        self.p1 = MaskedLazyLinear(**p1, mask=mask[1])
...
def main(config, args):
    ...
    torch.save(model.state_dict(), "C:/Users/tavar/Desktop/Thesis/Code/spiking/examples/torch/mnist/models/model_controller_basic.pth")
